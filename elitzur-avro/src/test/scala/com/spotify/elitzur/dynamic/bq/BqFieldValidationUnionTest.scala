...
class BqFieldValidationUnionTest extends AnyFlatSpec with Matchers with BeforeAndAfterEach {
  ...
  it should "correctly count the valid fields" in {
    ...
    val validRecord = new TableRow()
      .set("optRecord", new TableRow().set("optString", "PR"))
  ...
  it should "correctly count the invalid fields" in {
    ...
    val invalidRecord = new TableRow()
      .set("optRecord", new TableRow().set("optString", "XYZ"))
    ...
    (countryCodValidCount, countryCodInvalidCount) should be ((0, 1))
  }
