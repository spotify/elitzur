...
class AvroFieldExtractorBaseTest extends AnyFlatSpec with Matchers {
  ...
  it should "extract a primitive at the record root level" in {
    ...
    val fn = AvroObjMapper.getAvroFun(".userId", testSimpleAvroRecord.getSchema).accessorFn
  ...
  it should "extract an array at the record root level" in {
    ...
    val fn = AvroObjMapper.getAvroFun(".arrayLongs", testSimpleAvroRecord.getSchema).accessorFn
  ...
  it should "extract a nested record" in {
    ...
    val fn = AvroObjMapper.getAvroFun(".inner.userId", testSimpleAvroRecord.getSchema).accessorFn
  ...
  it should "extract a record if the field has _ in it" in {
    ...
    val fn = AvroObjMapper.getAvroFun("._user_id10", testSimpleAvroRecord.getSchema).accessorFn
