...
abstract case class DynamicValidationCompanion(validatorIdentifier: String) {
  val validator: Validator[_]
  def dynamicParser: Any => Any
}

object DynamicValidationCompanionImplicits {
  implicit class AugmentBaseCompanion[T, LT <: BaseValidationType[T]](c: BaseCompanion[T, LT]) {
    def dynamicParser(x: Any): Any = c.parse(x.asInstanceOf[T])
  }
}
