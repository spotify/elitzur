...
abstract class FieldValidator[A: ClassTag](implicit reporter: MetricsReporter)
    extends Validator[A] {
  ...
  override def validateRecord(
    ...
    val fieldConfig = config.fieldConfig(path)

    if (result.isValid) {
      if (fieldConfig != NoCounter) {
      ...
      if (fieldConfig == ThrowException) {
        throw new DataInvalidException(
          s"Invalid value ${result.forceGet.toString} found for field $path"
        )
      }
      if (fieldConfig != NoCounter) {
        ...
        if (!accessor.validator.isInstanceOf[IgnoreValidator[_]]) {
          ...
          val nextPath =
            if (accessor.validator.isInstanceOf[FieldValidator[_]]
                || accessor.validator.isInstanceOf[SeqLikeValidator[_, Seq]]) {
          ...
          val result = accessor.validator.validateRecord(
